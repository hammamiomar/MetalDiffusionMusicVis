...
from .sampler import SimpleEulerAncestralSampler, SimpleEulerSampler, _linspace
...
class StableDiffusion:
    ...
    def _init_walk_noise(
        ...
        shape = (n_images, *latent_size, self.autoencoder.latent_channels)
        walk_noise_x = mx.random.normal(shape)
        walk_noise_y = mx.random.normal(shape)
        
        walk_scale_x = mx.cos( _linspace(0,size,walk_steps) * math.pi)
        walk_scale_y = mx.sin( _linspace(0,size,walk_steps) * math.pi)
        
        noise_x = mx.tensordot(walk_scale_x, walk_noise_x, axes=0)
        noise_y = mx.tensordot(walk_scale_y, walk_noise_y, axes=0)
        
        noise = mx.add(noise_x,noise_y)
        
        return noise
        
    def generate_latents_from_noise(
        self,
        text: str,
        noise,
        n_images: int = 1,
        ...
        # Create the latent variables
        x_T = self.sampler.sample_prior_custom_noise(
            dtype=self.dtype,noise=noise
        )
